...
??? question "I deployed it but Cloudflare returns error 1101!"
    Your Cloudflare account has likely been flagged. Create a new Cloudflare account with an official email like Gmail. Also, make sure the project name doesn't include proxy-related keywords.
...
??? question "Why v2rayN cannot ping test configs?"
    Right now v2rayN is experiencing some issues with custom configs and Hamidreza Panel configs are all custom. No worries, just enable config and use it. You also have Best Ping config in all subscriptions which connects to best IP automatically, so you don't need to test all configs everytime.

??? question "Why sing-box throws error while importing subscription?"
    Hamidreza Panel only supports sing-box 1.12.0 or higher