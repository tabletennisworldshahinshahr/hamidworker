...
export function initHttp(request: Request, env: any) {
    ...
    if (!['/secrets', '/favicon.ico'].includes(decodeURIComponent(pathname))) {
        if (!UUID || !TR_PASS) throw new Error(`Please set ${_VL_CAP_} UUID and ${_TR_CAP_} password first. Visit <a href="${origin}/secrets" target="_blank">here</a> to generate them.`);
        if (!isValidUUID(UUID)) throw new Error(`Invalid UUID: ${UUID}`);
        if (typeof kv !== 'object') throw new Error(`KV Dataset is not properly set! Please refer to <a href="${_website_}" target="_blank">tutorials</a>.`);
    }
